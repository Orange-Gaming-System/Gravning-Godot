...
@export var gamecached = false
@export var gamecachefailure = ""
	...
	for n in commands.size(): #this loop runs for every line in the .grv file.
		...
		if "-" in mapargs[0]: #checks if this is for 1 map or a range of maps.
			...
			for map in int(maprange[1]) - int(maprange[0]) + 1: #for each map in the range, store them in their respective location in the array.
				map += int(maprange[0])
				mappaths[map-1] = "/".join([path.rsplit("/", true, 1)[0], mapargs[1].lstrip("\"")])
		else:
			mappaths[int(mapargs[0])-1] = "/".join([path.rsplit("/", true, 1)[0], mapargs[1].lstrip("\"")]) #if this is a single map, store its path in the correct location in the array.
